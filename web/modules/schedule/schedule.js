    ...
    async function fetchAllData() {
        try {
            const [scheduleRes, homeworkRes, reportRes] = await Promise.all([
                fetch("http://localhost:8000/schedules", { headers: { "Authorization": `Bearer ${token}` } }),
                fetch("http://localhost:8000/homeworks", { headers: { "Authorization": `Bearer ${token}` } }),
                fetch("http://localhost:8000/reports", { headers: { "Authorization": `Bearer ${token}` } })
            ]);
