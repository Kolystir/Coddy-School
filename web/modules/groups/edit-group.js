$(document).ready(function() {
    ...
    const API = {
        ...
        updateGroup:   id => `http://mature-nissy-kolystir-dbf3058a.koyeb.app:8000/groups/${id}`,
        deleteGroup:   id => `http://mature-nissy-kolystir-dbf3058a.koyeb.app:8000/groups/${id}`,
  ...
  <div class="row" id="groupsContainer">
    ${groups.map(g => `
      <div class="col-md-4 mb-4 group-card" data-name="${g.group_name.toLowerCase()}">
        ...
        $(document).on('click', '.editCourseButton', e => loadEditForm(+e.currentTarget.dataset.courseId));
        $(document).on('click', '.deleteCourseButton', e => {
        ...
        $('#deleteConfirmModal').on('hidden.bs.modal', enableScrolling);
    }
  ...
  <form id="editForm" class="shadow p-4 rounded bg-light">
      ...
      <select id="courseSelect" class="form-control" required>
        ...
        ${courses.map(c => `
          <option value="${c.course_id}"
            ${+c.course_id === +group.course_id ? 'selected' : ''}>
            ${c.course_name}
          </option>
        `).join('')}
      </select>
    ...
    <div class="d-flex justify-content-end gap-2">
      <button type="submit" class="btn btn-coddy">Сохранить</button>
        ...
        // При изменении курса — подставляем имя и пытаемся загрузить преподавателя
    ...
    // =========================
    // Утилита: показать текст (success | danger)
    // =========================
